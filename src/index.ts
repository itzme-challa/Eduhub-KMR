...
import { java } from './commands/java';
...
bot.command('broadcast', async (ctx) => {
  ...
  try {
    ...
    await ctx.reply('❌ Error fetching chat IDs.');
  }
});
...
bot.command('reply', async (ctx) => {
  ...
  if (!chatIdStr || !msgParts.length) return ctx.reply('Usage:\n/reply <chat_id> <message>');

  const chatId = Number(chatIdStr);
  if (isNaN(chatId)) return ctx.reply(`Invalid chat ID: ${chatIdStr}`);

  try {
    await ctx.telegram.sendMessage(chatId, `*Admin's Reply:*\n${msgParts.join(' ')}`, { parse_mode: 'Markdown' });
...
bot.command('users', async (ctx) => {
  ...
  try {
    const chatIds = await fetchChatIdsFromSheet();
    await ctx.reply(`👥 *Total Users:* ${chatIds.length}`, { parse_mode: 'Markdown' });
  } catch (err) {
    console.error('Users fetch error:', err);
    await ctx.reply('❌ Error fetching user list.');
  }
});
...
bot.on('message', async (ctx) => {
  ...
  if (msg.text?.startsWith('/contact')) {
    const contactMessage = msg.text.replace('/contact', '').trim() || msg.reply_to_message?.text;
    if (contactMessage) {
      await ctx.telegram.sendMessage(
        ADMIN_ID,
